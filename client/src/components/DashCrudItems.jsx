...
import { HiCheckCircle, HiXCircle, HiOutlineExclamationCircle, HiPlus, HiPencilAlt, HiTrash } from "react-icons/hi";
import { Table, Button, Modal, FileInput, Toast, Select, TextInput } from "flowbite-react";
...
const categories = [
  "Mobile Phones", "Laptops/Tablets", "Headphones/Earbuds", "Chargers and Cables", "Cameras", "Electronic Accessories",
  "Textbooks", "Notebooks", "Stationery Items", "Art Supplies", "Calculators", "Coats and Jackets", "Hats and Caps",
  "Scarves and Gloves", "Bags and Backpacks", "Sunglasses", "Jewelry and Watches", "Umbrellas", "Wallets and Purses",
  "ID Cards and Passports", "Keys", "Personal Care Items", "Sports Gear", "Gym Equipment", "Bicycles and Skateboards",
  "Musical Instruments", "Water Bottles", "Lunch Boxes", "Toys and Games", "Decorative Items", "Other"
];
...
export default function DashCrudItems() {
  ...
  const [itemToEdit, setItemToEdit] = useState({
    item: "", dateFound: "", location: "", description: "", imageUrls: [],
    category: "Other", status: "available", claimantName: "", claimedDate: ""
  });
  ...
  const handleSaveItem = async (e) => {
    ...
    try {
      const res = await fetch(`/api/items/${itemToEdit._id ? `updateItem/${itemToEdit._id}` : "report"}`, {
      ...
      if (res.ok) {
        if (itemToEdit.status === "claimed") {
          setClaimedItems((prev) => itemToEdit._id ? prev.map((item) => (item._id === itemToEdit._id ? data : item)) : [...prev, data]);
        } else {
          setItems((prev) => itemToEdit._id ? prev.map((item) => (item._id === itemToEdit._id ? data : item)) : [...prev, data]);
        }
        setFilteredItems((prev) => itemToEdit._id ? prev.map((item) => (item._id === itemToEdit._id ? data : item)) : [...prev, data]);
        setShowAddModal(false);
        setShowEditModal(false);
        setSuccessMessage(`Item ${itemToEdit._id ? "updated" : "added"} successfully.`);
        setTimeout(() => setSuccessMessage(""), 3000);
      } else {
        setErrorMessage(data.message);
        setTimeout(() => setErrorMessage(""), 3000);
  ...
  const handleEditItem = (item) => {
    setItemToEdit({ ...item, dateFound: new Date(item.dateFound).toISOString().split("T")[0] });
  ...
  return (
    <div className="container mx-auto p-3 scrollbar scrollbar-track-slate-100 scrollbar-thumb-slate-300 dark:scrollbar-track-slate-700 dark:scrollbar-thumb-slate-500">
      ...
      <div className="mb-4 w-full flex items-center justify-between">
        ...
        <Select color="gray" value={view} onChange={(e) => setView(e.target.value)}>
      ...
      <div className="overflow-x-auto">
        <Table hoverable className="min-w-full text-sm text-left text-gray-500 dark:text-gray-400">
          <Table.Head className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
            ...
            {view === "Claimed Items" && <Table.HeadCell>Claimant</Table.HeadCell>}
            {view === "Claimed Items" && <Table.HeadCell>Claimed Date</Table.HeadCell>}
          ...
          <Table.Body className="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
            {filteredItems.map((item) => (
              <Table.Row key={item._id} className="bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-600">
                <Table.Cell className="px-6 py-4">
                  <Link to={`/item/${item._id}`} className="font-bold">{item.item}</Link>
                </Table.Cell>
                <Table.Cell className="px-6 py-4">
                ...
                <Table.Cell className="px-6 py-4">{new Date(item.dateFound).toLocaleDateString()}</Table.Cell>
                <Table.Cell className="px-6 py-4">{item.department}</Table.Cell>
                <Table.Cell className="px-6 py-4">{item.status}</Table.Cell>
                {view === "Claimed Items" && <Table.Cell className="px-6 py-4">{item.claimantName}</Table.Cell>}
                {view === "Claimed Items" && <Table.Cell className="px-6 py-4">{new Date(item.claimedDate).toLocaleDateString()}</Table.Cell>}
                {view === "All Items" && (
                  <Table.Cell className="px-6 py-4">
                    <div className="flex items-center ml-auto space-x-2 sm:space-x-3">
                      ...
                      <Button color="failure" onClick={() => { setShowModal(true); setItemIdToDelete(item._id); }}>
      ...
      {/* Add Item Modal */}
      <Modal show={showAddModal} onClose={() => setShowAddModal(false)} size="2xl">
        ...
        <Modal.Body>
          <form onSubmit={handleSaveItem}>
            <div className="grid grid-cols-6 gap-6">
              ...
              <div className="col-span-6 sm:col-span-3">
                <label htmlFor="dateFound" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Date Found</label>
                <input
                  type="date"
                  name="dateFound"
              ...
              <div className="col-span-6">
                <label htmlFor="imageUrls" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Image URLs</label>
                <FileInput
                  type="file"
                  id="images"
                  accept="image/*"
                ...
                >
                  {imageUploadProgress ? `Uploading ${imageUploadProgress}%` : "Upload Images"}
                </Button>
              </div>
            </div>
            <div className="items-center p-6 border-t border-gray-200 rounded-b dark:border-gray-700">
              <Button type="submit" gradientDuoTone="pinkToOrange" className="w-full">Save all</Button>
            </div>
          </form>
        </Modal.Body>
      </Modal>
